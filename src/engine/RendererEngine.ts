import type { FormNode, FieldNode, ContainerNode, LayoutNode, StaticNode, FormDefinition } from "../types/dsl";
import type {
...
export class RendererEngine<T = any> implements IRendererEngine<T> {
  ...
  render(node: FormNode, ctx: RenderContext<T>): T {
    const children = node.category !== "field" && (node as any).children
