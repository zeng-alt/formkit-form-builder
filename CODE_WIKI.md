...
*   **UI 体系**：
    *   采用了类似 `shadcn-vue` 的无头组件设计模式，底层无头组件使用了 **Reka UI**。
    *   样式由 **Tailwind CSS v4** 驱动（结合 `clsx` 和 `tailwind-merge` 管理动态类名）。
    *   部分复杂的布局和即用型组件（如侧边栏布局、弹窗、提示框等）混用了 **Naive UI**。
...
*   基于 Reka UI 和 Tailwind 封装的大量基础原子组件（Button, Input, Select, Dialog, Tooltip 等），主要供构建器自身界面使用，保证了高可定制性和一致的设计语言。
...
*   **UI 与表单框架**：
    ...
    *   `naive-ui` (^2.44.1)（辅助 UI 库）
    *   `reka-ui` (^2.9.5)（无头 UI 库）
*   **样式体系**：
